...
\begin{itemize}
  \item \textbf{Détection et coïncidence}  
    Mettre en place un télescope à scintillateurs plastiques et isoler les muons réels du bruit de fond.
  \item \textbf{Distribution angulaire}  
    Mesurer le flux en fonction de l’angle et vérifier la loi $I(\theta)\propto\cos^2\!\theta$.
  \item \textbf{Dépôt d’énergie}  
    Étudier le pouvoir de pénétration des muons à l’aide d’absorbeurs de plomb.
  \item \textbf{Temps de vie du muon}  
    Chronométrer la désintégration des muons arrêtés pour déterminer $\tau_\mu$.
  \item \textbf{Modernisation technique}  
    Adopter des détecteurs SiPM et un traitement FPGA pour gagner en compacité et en rapidité.
\end{itemize}
...
Un muon traversant un scintillateur plastique perd de l’énergie par collisions avec les atomes du milieu. Cette énergie est convertie rapidement en photons dans l’UV/visible. La plupart de ces photons restent piégés par réflexion interne et sont récoltés par le photomultiplicateur placé en contact optique.

\vspace{1em}
\noindent
\textbf{Suggérer ici un schéma :} \emph{“Schéma global d’un scintillateur couplé à un photomultiplicateur. Illustration disponible sur~:}
\url{https://commons.wikimedia.org/wiki/File:Photomultiplier_tube_schematic.png}
\emph{.”}
...
\subsection{Chaîne d’électronique rapide}
\begin{itemize}
  \item \textbf{Amplificateur et discriminateur} : L’amplificateur augmente l’amplitude du signal du PM. Le discriminateur applique ensuite un seuil pour ne délivrer qu’un signal logique standard (NIM ou TTL).  
  \item \textbf{Logique de coïncidence} : Les signaux issus de plusieurs détecteurs (ex. plusieurs scintillateurs) sont combinés par porte AND (coïncidence), OR, etc. Une coïncidence valide indique le passage simultané d’un muon à travers plusieurs plaques.  
  \item \textbf{Modules de mesure} : Pour mesurer le temps de vol ou de vie, on utilise un “Time-to-Amplitude Converter” (TAC) convertissant un intervalle de temps en amplitude électrique, analysable par un convertisseur analogique-numérique.  
  \item \textbf{Système d’acquisition} : Un compteur/timer NIM ou un dispositif piloté par PC (LabVIEW, etc.) enregistre le nombre de coïncidences ou les histogrammes temporels.  
\end{itemize}

\vspace{1em}
\noindent
\textbf{Suggérer ici un schéma :} \emph{“Diagramme d’architecture de l’électronique SiPM/PM + discri + coïncidence + acquisition. Par exemple :}
\url{https://www.nupecc.org/presentations/pictures/detector_electronics_diagram.png}
\emph{.”}

\subsection{Montage en télescope à muons}
Pour l’étude angulaire, deux scintillateurs identiques sont superposés à une distance fixe et montés sur une structure orientable. L’idée est de changer l’angle $\theta$ pour mesurer la répartition angulaire du flux muonique. 

Pour la mesure du temps de vie, on ajoute un troisième scintillateur faisant office de “stop” : un muon y est mesuré lorsqu’il s’y arrête, et sa désintégration déclenche un second signal (start-stop sur le TAC).

\vspace{1em}
\noindent
\textbf{Suggérer ici une photo du montage :}  
\emph{“Photo du télescope à 3 plaques scintillantes, aimablement fournie par le laboratoire, ou trouvée sur :}
\url{https://commons.wikimedia.org/wiki/File:Muon_telescope_example.jpg}
\emph{.”}

\subsection{Réglages et étalonnages}
\begin{itemize}
  \item \textbf{Haute tension} : Le PM nécessite typiquement 1000 V. On ajuste cette tension pour un gain optimal.  
  \item \textbf{Seuil du discriminateur} : Un réglage adéquat permet de filtrer le bruit de fond tout en conservant les impulsions des muons.  
  \item \textbf{Synchronisation} : On égalise les longueurs de câbles et on peut insérer des retards pour aligner les impulsions dans la fenêtre de coïncidence à quelques nanosecondes près.  
  \item \textbf{Calibration} : Possible via une source radioactive bêta/gamma ou en étudiant la variation du taux de comptage en fonction de la haute tension et du seuil.  
\end{itemize}

Tous ces réglages garantissent une coïncidence fiable, traduisant le passage réel d’un même muon à travers plusieurs détecteurs presque simultanément.
...
Technique TAC et limites : Le recours au TAC s’est avéré efficace pour mesurer des temps de l’ordre de la microseconde avec une bonne résolution (~10 ns). Nous avons tout de même dû tenir compte de sa limite de durée : notre TAC était réglé pour une pleine échelle de 10 μs (par exemple), ce qui signifie qu’au-delà de 10 μs sans stop, il arrête la conversion (soit en saturant, soit en annulant l’événement). Ainsi, les muons dont la désintégration prend plus de cette fenêtre d’observation ne sont pas comptés correctement. Cependant, étant donné que 10 μs correspond à plus de 4 fois $\tau$, la fraction de muons se désintègrant plus tard est faible ($e^{-10/2,2} \approx 0,006$ soit <1%). Nous avons donc négligé l’effet de coupure à 10 μs sur l’ajustement, ou le cas échéant, nous l’avons inclus dans le fond constant.
