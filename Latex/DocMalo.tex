...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Images/Montage_Electrique.png}
...
L’analyse des données repose sur l’ajustement de la courbe $N(x)$ selon une loi de puissance dérivée du spectre attendu des muons cosmiques : $N(E_{\min}) \propto E_{\min}^{1 - \gamma}$, avec $\gamma \approx 2.7$. Ce modèle permet alors d’estimer l’énergie moyenne des muons détectés via
\[
\langle E \rangle \approx \frac{\gamma - 1}{\gamma - 2} \cdot E_{\min,\max},
\]
...
\begin{figure}[!h]
  ...
  {0.45\textwidth}
  \centering
  \includegraphics[width=0.7\textwidth]{Images/energie.png}
...
Enfin, des incertitudes statistiques (bruit de comptage de type Poissonien) et systémiques (approximation de $dE/dx$, fluctuations d’énergie déposée, biais de sélection) limitent la précision de la mesure. Néanmoins, cette approche permet une estimation réaliste de l’ordre de grandeur de l’énergie des muons cosmiques (typiquement quelques centaines de MeV à quelques GeV), tout en illustrant des effets de physique des particules dans la matière.
