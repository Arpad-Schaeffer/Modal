...
% Encadré clé sur le but du montage expérimental
...
\subsubsection*{Scintillateur plastique}
...
% --- Chaîne électronique (fusion docMalo + doc.tex) ---
...
% --- Schémas/images de doc.tex (à la place des figures vides de docMalo) ---
...
% ...suite du texte docMalo...

\newpage

\section{Étude du signal}

\subsection{Observation du signal et décroissance temporelle}
...
\begin{figure}[H]
    \centering
    %\includegraphics[width=0.7\textwidth]{chemin/vers/graphique_signal.png}
...
\begin{remarque}
\textbf{Synthèse~:} L’observation du signal permet de caractériser la réponse temporelle du scintillateur et du circuit RC. Les constantes de temps mesurées (à compléter avec les valeurs expérimentales) serviront de référence pour l’optimisation des réglages et l’interprétation des mesures ultérieures.
\end{remarque}
...
Pour optimiser la détection des muons, nous avons étudié simultanément l’influence de la tension d’alimentation du photomultiplicateur (PM) et du seuil du discriminateur sur le taux de détection.
...
\paragraph{Méthode~:} Pour chaque scintillateur/PM, nous avons fait varier la haute tension (HT) appliquée et, pour chaque valeur de HT, nous avons balayé différents seuils du discriminateur. À chaque combinaison, nous avons mesuré le nombre de signaux détectés par unité de temps.

\paragraph{Théorie~:} Une tension trop basse conduit à un gain insuffisant~: peu de signaux dépassent le seuil du discriminateur. À l’inverse, une tension trop élevée augmente le gain mais aussi le bruit de fond (courant d’obscurité, bruit thermique), ce qui accroît le nombre de faux signaux. Un seuil trop bas laisse passer tout le bruit, tandis qu’un seuil trop haut élimine aussi les vrais signaux de muons. L’optimisation consiste donc à trouver la zone où le taux de vrais signaux est maximal et le bruit minimal.

\paragraph{Résultats~:} On observe une zone optimale pour chaque PM où le nombre de signaux “vrais” est maximal, avant que le bruit ne devienne prépondérant. La courbe typique montre beaucoup d’événements à seuil bas (bruit dominant), puis un plateau correspondant aux vrais signaux, enfin une chute rapide lorsque le seuil dépasse l’amplitude des signaux muoniques. Le choix de la tension optimale et du seuil est donc un compromis entre sensibilité et bruit.

\begin{figure}[H]
    \centering
    %\includegraphics[width=0.7\textwidth]{chemin/vers/graphique_HT.png}
    \caption{Nombre de signaux détectés en fonction de la haute tension appliquée au PM et du seuil du discriminateur.}
\end{figure}

\paragraph{Conclusion~:} Nous avons retenu, pour chaque PM, une valeur de HT assurant un taux de détection élevé tout en limitant les faux positifs, et un seuil situé juste au-dessus du niveau de bruit, de façon à ne conserver que les signaux dus aux muons.

\begin{remarque}
\textbf{Synthèse~:} Un réglage optimal et conjoint de la haute tension et du seuil permet d’obtenir un signal fiable, en séparant efficacement les vrais événements du bruit.
\end{remarque}
...
% Encadré clé sur le principe du montage

\vspace{1em}
\begin{center}
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black, title=Principe de la détection par coïncidence]
Utiliser la coïncidence temporelle entre plusieurs détecteurs pour isoler les vrais muons et éliminer le bruit de fond, garantissant la pureté du signal.
\end{tcolorbox}
\end{center}
...
\vspace{1em}
\begin{remarque}
\textbf{Synthèse~:} La coïncidence à deux scintillateurs améliore fortement la fiabilité de la détection des muons cosmiques en supprimant la majorité du bruit de fond. Ce principe permet d’obtenir un rapport signal/bruit élevé, condition indispensable pour des mesures quantitatives précises du flux muonique.
\end{remarque}
...
\begin{remarque}
\textbf{Synthèse~:} La coïncidence améliore considérablement le rapport signal/bruit et permet des mesures précises du flux muonique.
\end{remarque}

\newpage

\section{Distribution angulaire du flux muonique}
L'un des caract\`eres notables du rayonnement cosmique muonique au niveau du sol est son anisotropie en angle \text{\'enithal}. Les muons proviennent majoritairement de la verticale, c'est-\`a-dire de la direction du \text{\'enith} (car ils g\'en\'e\'res en haute atmosph\`ere et suivent grossi\`erement des trajectoires quasi-rectilignes vers le bas). Plus l'angle~$\theta$ par rapport \`a la verticale augmente (muons venant rasants de l'horizon), plus le parcours dans l'atmosph\`ere est long, et donc plus la probabilit\'e de survie du muon jusqu'au sol diminue. Ce ph\'enom\`ene se traduit par une loi empirique proche de
\[
...
\vspace{1em}
\begin{center}
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black, title=Principe de la distribution angulaire]
Étudier la variation du flux de muons en fonction de l’angle d’incidence pour valider expérimentalement la loi angulaire attendue et comprendre l’anisotropie du rayonnement cosmique.
\end{tcolorbox}
\end{center}
...
\]
avec $\Lambda_{\mu}$ la longueur d'atténuation (liée à la longueur de désintégration $\gamma c\tau$ des muons, de l'ordre de~6 km d'équivalent air pour des muons de quelques GeV). Pour des angles zénithal~$\theta$, la colonne d'air parcourue est approximativement
\[
X(\theta)=\frac{X(0)}{\cos\theta}
\]
(pour $\theta$ pas trop grand et en négligeant la courbure terrestre), où $X(0)$ est l'épaisseur verticale correspondant à 1013 hPa (environ 1030 g/$cm^2$ d'équivalent masse d'air). La probabilité de survie d'un muon jusqu'au sol décroît exponentiellement avec~$X$:
...
\begin{remarque}
\textbf{Synthèse~:} La mesure de la distribution angulaire confirme la loi en $\cos^2\theta$ et illustre l’influence de l’atmosphère sur le flux muonique.
\end{remarque}
...
\begin{remarque}
\textbf{Synthèse~:} L’expérience met en évidence la grande pénétration des muons et la distribution typique du dépôt d’énergie, validant les modèles théoriques.
\end{remarque}
...
Mesurer expérimentalement la durée de vie propre des muons arrêtés dans un absorbeur, pour valider la décroissance exponentielle et la relativité restreinte.
\end{tcolorbox}
\end{center}
...
\begin{remarque}
\textbf{Synthèse~:} La mesure du temps de vie du muon confirme la validité du modèle standard et la dilatation relativiste du temps. Les résultats sont en accord avec les valeurs de référence, confirmant la méthode expérimentale.
\end{remarque}
...
\begin{remarque}
\textbf{Synthèse~:} La modernisation technique par l’utilisation de SiPM et FPGA a amélioré la compacité, la portabilité et la facilité d’utilisation du dispositif, tout en maintenant la précision des mesures.
\end{remarque}
