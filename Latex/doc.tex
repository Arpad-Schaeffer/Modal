...
\begin{figure}[H]
    \centering
    %\includegraphics[width=0.7\textwidth]{Images/optimisation_scint1.png}
    \fbox{\parbox{0.7\textwidth}{\centering\textit{Graphique à insérer : Scintillateur 1}}}
    %\includegraphics[width=0.7\textwidth]{Images/optimisation_scint2.png}
    \fbox{\parbox{0.7\textwidth}{\centering\textit{Graphique à insérer : Scintillateur 2}}}
    %\includegraphics[width=0.7\textwidth]{Images/optimisation_scint3.png}
    \fbox{\parbox{0.7\textwidth}{\centering\textit{Graphique à insérer : Scintillateur 3}}}
    \caption{Nombre d’événements détectés en fonction du seuil pour différentes HT, pour chaque scintillateur.}
    \label{fig:optimisation_scint}
\end{figure}

\paragraph{Analyse et synthèse~:}
L’analyse croisée des courbes obtenues permet de dégager, pour chaque scintillateur, un couple (HT, seuil) optimal :
\begin{itemize}
...
\end{itemize}
Ces réglages assurent un taux de détection élevé tout en limitant les faux positifs. Ils constituent la base des mesures ultérieures sur les muons cosmiques.

% \vspace{1em}
% \textbf{Remarque~:} Les valeurs optimales sont à adapter selon les caractéristiques individuelles de chaque couple scintillateur/PM.

% --- TRANSITION FLUIDE ---

\vspace{1em}
La section suivante détaille la mise en œuvre de la détection par coïncidence, étape clé pour isoler les vrais événements muoniques du bruit de fond résiduel.
\newpage
