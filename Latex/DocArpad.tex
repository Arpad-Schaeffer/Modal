...
\begin{remarque}
\textbf{Résumé~:} Les scintillateurs plastiques permettent une détection rapide, fiable et économique des muons cosmiques. Lorsqu’un muon les traverse, il perd de l’énergie par ionisation, rapidement convertie en photons UV/visible. Grâce à la réflexion interne, une grande partie de cette lumière est guidée vers le photodétecteur, assurant une collecte efficace du signal.
\end{remarque}
...
\begin{remarque}
\textbf{Résumé~:} Les tubes photomultiplicateurs restent la référence pour la détection de faibles signaux lumineux en physique des particules, grâce à leur gain élevé, leur rapidité et leur faible bruit. Toutefois, ils nécessitent une alimentation haute tension et sont sensibles aux champs magnétiques et à la lumière ambiante.
\end{remarque}
...
\begin{remarque}
\textbf{Résumé~:} Le circuit électronique permet de récupérer le signal issu du photomultiplicateur, de l’afficher (oscilloscope) et de l’exploiter pour compter le nombre de muons, détecter les coïncidences, éliminer le bruit de fond, et mesurer le temps de vie des muons.
\end{remarque}
...
\begin{figure}[H]
    \centering
    %\includegraphics[width=0.7\textwidth]{chemin/vers/graphique_threshold.png}
